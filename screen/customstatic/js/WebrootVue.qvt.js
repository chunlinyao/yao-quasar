    ...
    methods: {
        ...
        reLoginCheckResponseSuccess: function(resp, status, jqXHR) {
            if (resp.username && resp.sessionToken) {
                ...
                var msg = '在另一个标签页登录后会话已经更新，没有发生变更，请重试';
        ...
        */
        ...
        reLoginReload: function () {
            if (confirm("重新加载页面吗? 所有没保存的修改会丢失。"))
...
Vue.component('m-luckysheet', {
    ...
    mounted: function() {
        ...
        vm.keydownHandler = function(evt){
            if(evt.key == 'ArrowRight' && getCaretPosition(this) == this.textContent.length) {
